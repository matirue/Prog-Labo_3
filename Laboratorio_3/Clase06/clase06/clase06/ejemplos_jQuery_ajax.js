...
// y recuerda el objeto jqxhr de la solicitud
	var jqxhr = $.ajax("BACKEND/ajax_test.php")
		.done(function() {//.done reemplaza el check si la funcion devueve status 200 y 4 y ejecuto los {}
			alert("done");
		})//en caso de falla se ejecuta el fail
		.fail(function(jqXHR, textStatus, errorThrown) {
			alert(jqXHR.responseText + "\n");
			alert(textStatus + ":\n" + errorThrown);
		})//exito o no el .always siempre se ejecuta
		.always(function() {
...
function JqueryAjaxRetornoJSON(){//peticiones cuyo retorno son obj json
    ...
    $.ajax({
		...
		//no tiene data xq no le paso nada 
    })
	.done(function (objJSON) {//como datatype es de tipo json, el done internamente hace un pase de json automaticamente
		//MUESTRO EL RESULTADO DE LA PETICION
